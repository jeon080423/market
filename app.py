...
def load_data():
    ...
    sector_data = yf.download(list(sector_tickers.values()), period="5d")['Close']
    
    return kospi, sp500, nikkei, exchange_rate, us_10y, us_2y, vix, copper, freight, wti, dxy, sector_data, sector_tickers
...
try:
    ...
    with r1_c1: st.plotly_chart(create_chart(sp_s, "미국 S&P 500", sp_s.last('365D').mean()*0.9, ""), use_container_width=True)
    with r1_c2: st.plotly_chart(create_chart(fx_s, "원/달러 환율", fx_s.last('365D').mean()*1.02, ""), use_container_width=True)
    with r1_c3: st.plotly_chart(create_chart(cp_s, "구리 가격 (Copper)", cp_s.last('365D').mean()*0.9, ""), use_container_width=True)

    r2_c1, r2_c2, r2_c3 = st.columns(3)
    with r2_c1: st.plotly_chart(create_chart(yield_curve, "장단기 금리차", 0.0, ""), use_container_width=True)
    with r2_c2:
    ...
    with r2_c3: st.plotly_chart(create_chart(vx_s, "VIX 공포 지수", 30, ""), use_container_width=True)

    st.markdown("---")
    r3_c1, r3_c2, r3_c3 = st.columns(3)
    with r3_c1: st.plotly_chart(create_chart(fr_s, "글로벌 물동량 (BDRY)", fr_s.last('365D').mean()*0.85, ""), use_container_width=True)
    with r3_c2: st.plotly_chart(create_chart(wt_s, "에너지 가격 (WTI)", wt_s.last('365D').mean()*1.2, ""), use_container_width=True)
    with r3_c3: st.plotly_chart(create_chart(dx_s, "달러 인덱스 (DXY)", dx_s.last('365D').mean()*1.03, ""), use_container_width=True)

    # 10. (4번 기능 추가) 섹터별 순환매 히트맵
    st.markdown("---")
    st.subheader("🌡️ 섹터별 자금 흐름 히트맵 (KOSPI 주요 섹터)")
    ...
    if not df_perf.empty:
        ...
        st.info("**분석 가이드**: 종합 위험 지수가 상승할 때 '유틸리티', '금융' 등 방어 섹터가 상대적으로 선방하는지, '반도체'나 '2차전지' 같은 민감 섹터에서 자금이 빠져나가는지 확인하세요.")

except Exception as e:
    st.error(f"오류 발생: {str(e)}")

st.caption(f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | 섹터 순환매 분석 모듈 활성화")
