...
def render_vincent_valuation_page():
    ...
    # 연간 컬럼 분리 (네이버 테이블 기준 처음 4개가 연간 지표)
    cols_annual = cols[:4]

    # 키 매핑 유연화 (네이버 실적 분석 기준 키 매칭)
    eps_key = next((k for k in financials.keys() if 'EPS' in k), None)
    per_key = next((k for k in financials.keys() if 'PER' in k), None)
    sales_key = next((k for k in financials.keys() if '매출액' in k), None)
    op_key = next((k for k in financials.keys() if '영업이익' in k and '영업이익률' not in k), None)
    ...
    # 과거 평균 PER 구하기 (E가 아닌 연도들의 PER 값 평균)
    ...
    with col_sim2:
        # 4.2 Target PER 슬라이더 조절
        # 과거 평균 PER 또는 예상 PER를 슬라이더 초기값으로 힌트 제공
    ...
    with m_col1:
        st.markdown(f"""
            <div class="metric-card">
                ...
                <div class="metric-value" style="color: #2b2b2b;">{curr_price:,.0f}원</div>
