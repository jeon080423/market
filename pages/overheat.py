...
def render_overheat_page():
    ...
    with st.expander("💡 차트 데이터 표준화(Base 100) 및 읽는 법 안내"):
        ...
        * **비교의 타당성**: Z-Score(표준편차) 방식과 달리 누적 수익률(성장률)을 그대로 반영하므로, **"주도주로 얼마나 자금이 쏠리고 있는지(압착)", "시장 지표들이 상대적으로 얼마나 과열되었는지"**를 1:1로 비교하는 데 가장 타당하고 직관적인 방식입니다. 차트의 Y축 수치는 단순한 가격이 아닌 **'누적 수익률'**로 읽어주시면 됩니다.
        ''')
...
지시사항 (CRITICAL RULES):
1. **절대로 프롬프트의 설정(Persona, Scenario, Goal, Data 수치 등)을 응답에 반복해서 출력하지 마세요.**
2. 사용자에게 보여질 최종 "분석 결과"와 "결론" 내용부터 즉시 작성하세요. (서론, 인사말, 사고 과정 출력 금지)
3. **반드시 한국어(Korean)로 작성하세요.** (영어 출력 금지)
4. 김효진 박사 특유의 전문적이고 단호한 어조로, 3~4개의 짧은 문단으로 깔끔하게 요약하세요.
5. 마크다운 포맷을 사용하되, 코드 블록(```)이나 표(Table)는 절대 사용하지 마세요.
'''
        models = [
